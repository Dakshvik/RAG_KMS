...
# --- WINDOWS RTX 4060 8GB OPTIMIZATIONS ---
# 1. Force PyTorch to use CUDA
os.environ["TORCH_DEVICE"] = "cuda"

# 2. Prevent VRAM fragmentation spikes (Crucial for 8GB cards)
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True,max_split_size_mb:64"

# 3. Limit Surya OCR batch size to prevent OOM on dense images
os.environ["SURYA_BATCH_SIZE"] = "2" 

# 4. Saturate the 20-Core CPU (leaving 4 cores for background/GPU handoffs)
...
class ProductionPDFExtractor:
    def __init__(self):
        # 1. Fast Path: Docling (CPU ISOLATED)
        logger.info("Initializing Docling models (Isolated to 20-Core CPU)...")
        pipeline_options = PdfPipelineOptions()
        pipeline_options.do_ocr = False # Fast path doesn't need OCR
        pipeline_options.accelerator_options = AcceleratorOptions(
            num_threads=16, 
            device="cpu" # CRITICAL: Keeps Docling out of VRAM
        )
        ...
        # 2. Slow Path: Marker (GPU ISOLATED)
        logger.info("Initializing Marker models (CUDA - RTX 4060)...")
        # Ensure PyTorch creates fp16 models to save massive amounts of VRAM
        with torch.autocast("cuda", dtype=torch.float16):
            self.artifact_dict = create_model_dict()
            self.marker_converter = PdfConverter(artifact_dict=self.artifact_dict)
    ...
    def process_fast_path(self, pdf_path: Path) -> List[Document]:
        logger.info(f"⚡ Fast Path (CPU Docling): {pdf_path.name}")
        ...
        if is_text_corrupted(sample_text):
            logger.warning(f"🚨 Corrupted font detected in {pdf_path.name}. Rerouting to Marker (GPU).")
        ...
        for idx, chunk in enumerate(docling_chunks):
            ...
            metadata = {
                ...
                "source": str(pdf_path).replace("\\", "/"), # Normalize slashes
    ...
    def process_slow_path(self, pdf_path: Path, language: str, method: str) -> List[Document]:
        logger.info(f"🏎️ GPU Path (CUDA Marker): {pdf_path.name}")
        
        text = ""
        rendered = None
        try:
            # Run inference in fp16 mixed precision to save VRAM
            with torch.autocast("cuda", dtype=torch.float16):
            ...
            # 🛑 VRAM SPIKE PROTECTION: Aggressive Cleanup 
            del rendered
            gc.collect()
