...
class HybridRAGPipeline:
    ...
    # ---------- Key term extraction and filtering (NEW) ----------
    def extract_key_terms(self, query: str) -> List[str]:
        """Extract acronyms and capitalised multi‑word phrases from the query."""
        terms = []
        # All‑caps acronyms (e.g., BNSS, IPC, POCSO)
        acronyms = re.findall(r'\b[A-Z]{2,}\b', query)
        terms.extend(acronyms)
        # Capitalised multi‑word phrases (e.g., Dowry Prohibition Act)
        phrases = re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+\b', query)
        terms.extend(phrases)
        # Also catch single capitalised words that are at least 3 letters (like "Dowry")
        single_caps = re.findall(r'\b[A-Z][a-z]{2,}\b', query)
        terms.extend(single_caps)
        # Lowercase, deduplicate, remove very short terms
        unique_terms = list({t.lower() for t in terms if len(t) > 1})
        return unique_terms

    def keyword_filter_multi(self, points: List[Any], terms: List[str]) -> List[Any]:
        """Keep points whose text or filename contains any of the given terms (case‑insensitive)."""
        if not terms:
            return points
        filtered = []
        for p in points:
            text = p.payload.get("text", "").lower()
            fname = p.payload.get("filename", "").lower()
            if any(term in text or term in fname for term in terms):
                filtered.append(p)
        return filtered
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        # ---- Generic keyword boost (replaces hardcoded BNSS block) ----
        key_terms = self.extract_key_terms(query)
        if key_terms:
            filtered = self.keyword_filter_multi(all_points, key_terms)
            if filtered:
                logger.info(f"Filtered to chunks mentioning: {key_terms}")
                all_points = filtered
            else:
                logger.info("No chunks matched key terms; using original retrieval.")
        # ---------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    test_queries = [
        ...
        "How many years of imprisonment and how much fine should we pay if we take dowry?"
