...
# # ---------- XML SYSTEM PROMPTS ----------
# MASTER_SYSTEM_PROMPT = """<role>Legal/Kanoon AI – specialised assistant for the Indian Police</role>
# <subjects>
...
# - If question is outside the above subjects, reply: "I can only answer police/legal questions."
# </rules>"""

# FACTUAL_PROMPT_TEMPLATE = """<type>factual</type>
# <instruction>
# You are an expert knowledge assistant. Answer the user's question using ONLY the provided CONTEXT.
# Be brief, precise, and highly factual. Extract the exact answer without adding any outside knowledge.
# Respond entirely in {lang}.
# CRITICAL: Do not write out internal thoughts or drafting processes. Output the final answer immediately.
# </instruction>"""

# EXPLANATORY_PROMPT_TEMPLATE = """<type>explanatory</type>
# <instruction>
# You are an expert knowledge assistant. Explain the concepts asked by the user clearly, using ONLY the provided CONTEXT.
# Synthesize the steps or concepts logically to help the user understand.
# Respond entirely in {lang}. Do not hallucinate or use outside knowledge.
# CRITICAL: Do not write out internal thoughts or drafting processes. Output the final answer immediately.
# </instruction>"""

# REWRITE_PROMPT_TEMPLATE = """<task>Rewrite the user's question into a single, factual sentence that includes the most likely legal section, act name, punishment amount (imprisonment and/or fine) if the user asks about penalties. If the question is about a procedure, include key legal terms. Do not answer the question – only restate it in a more specific, searchable form. Respond in {lang}.</task>"""
...
#     # ---------- Query Rewriting (improved, no hardcoding) ----------
#     def rewrite_query(self, query: str, lang: str) -> List[str]:
#         system = REWRITE_PROMPT_TEMPLATE.format(lang=lang)
#         try:
#             resp = ollama.chat(
#                 model=LLM_FAST,
#                 messages=[
...
#     # ---------- Reranking ----------
#     def rerank_chunks(self, query: str, points: List[Any], top_k: int = 10) -> List[Any]:
#         if not points:
#             return []
...
#     # ---------- Context Compression (disabled, kept for optional use) ----------
...
#         # Choose the appropriate XML template and fill in the language
#         if query_type == "factual":
#             dynamic_prompt = FACTUAL_PROMPT_TEMPLATE.format(lang=lang)
#         else:
#             dynamic_prompt = EXPLANATORY_PROMPT_TEMPLATE.format(lang=lang)
...
#         # Trigger rewrite for any explanatory or punishment-related query
#         if any(kw in query.lower() for kw in [
#             "explain", "how", "why", "describe", "compare",
#             "punishment", "imprisonment", "fine", "years", "penalty",
#             "వివరించు",
#         ]):
...
#             points = self.hybrid_retrieve(sq, language_filter=lang_filter, limit=50)
...
#         t0 = time.perf_counter()
#         top_points = self.rerank_chunks(query, all_points, top_k=10)
#         timings["reranking"] = round(time.perf_counter() - t0, 3)

#         context = self.build_context(top_points)

#         # Compression disabled – the model can handle large context
#         # t0 = time.perf_counter()
...
#     test_queries = [
#         "How how much fine should we pay if anyone demands dowry as per dowry prohibition act?"
#     ]
...
class HybridRAGPipeline:
    ...
    # ---------- Query Rewriting (only for explanatory queries) ----------
    def rewrite_query(self, query: str, lang: str) -> List[str]:
        system = REWRITE_PROMPT_TEMPLATE.format(lang=lang)
        try:
            resp = ollama.chat(
    ...
    # ---------- Context Compression (re‑enabled with moderate threshold) ----------
    def compress_context(self, query: str, context: str, lang: str) -> str:
        if len(context) <= 2000:
            return context
        system = (
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        t0 = time.perf_counter()
        search_queries = [query]
        # Only rewrite for truly explanatory queries – simple fact lookups stay as‑is
        if any(kw in query.lower() for kw in ["explain", "why", "describe", "compare", "వివరించు"]):
            search_queries = self.rewrite_query(query, lang)
        timings["rewriting"] = round(time.perf_counter() - t0, 3)
        ...
        all_points = []
        for sq in search_queries:
            points = self.hybrid_retrieve(sq, language_filter=lang_filter, limit=30)   # reduced from 50
            all_points.extend(points)
        unique = {}
        for p in all_points:
            unique[p.id] = p
        ...
        t0 = time.perf_counter()
        top_points = self.rerank_chunks(query, all_points, top_k=5)   # reduced from 10
        timings["reranking"] = round(time.perf_counter() - t0, 3)

        context = self.build_context(top_points)

        # Compression enabled with moderate threshold – keeps context focused
        t0 = time.perf_counter()
        if len(context) > 5000:
            context = self.compress_context(query, context, lang)
        timings["compression"] = round(time.perf_counter() - t0, 3)
